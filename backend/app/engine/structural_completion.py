...
def _sync_composition_parents(
    ...
    """Post-pass C: When a child capability is CREATE/DELETE, ensure parent page is MODIFY.

    This ensures the parent page is regenerated by the renderer with correct
    composition (imports/JSX references) after children are added or removed.

    Rules:
      1. CREATE/DELETE on a child → parent page MODIFY (regenerate with composition)
    ...
    for rc in resolved:
        if rc.action not in (CREATE, DELETE):
            continue
        ...
        warnings.append(
            f"{parent_cap}: composition sync ({rc.action} {rc.name})"
        )
